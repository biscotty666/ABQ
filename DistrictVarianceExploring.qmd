...
subtitle: "Visualization and Spatial Autocorrelation"
format: 
  html:
    toc: true
  gfm: default
...
```{r}
var_maps <- map2(seq_len(length(lm_plots)), var_names,
    \(x, y) vmaps[[x]] + lg_plots[[x]] + 
      plot_annotation(title = glue("Albuquerque, NM - {y} - SA")))
...
```{r}
purrr::walk2(
  var_names, seq_len(length(var_names)),
  \(x, y) ggsave(paste0("images/", x, ".png"), var_maps[[y]])
)
```
