...
```{r}
#| warning: false
council_dists <- st_read("data/abq_demographics_2023.gpkg", 
                        layer = "council_dists") %>% 
...
I'll use the `split_dists` function from last time. I will also combine male and female. I re-ran the functions for 2024 in order to compare the two years. Since I will be doing the same thing later for education, I'll make functions to process the data.

The `split_dists` function leaves numerous "fragments" of tracts which are too small to be assigned any data. I will remove any records which represent less than 1% of the area of a given tract.

I'll use the `%<-%` function from `zeallot` to create and modify data frames. `map` and its variants return lists. By providing a vector of variable names on the left hand side of `%<-%`, `map`'s list will be de-structured and assigned to the variable names. 

```{r}
#| warning: false
#| message: false
c(age_sex_bern_2023, age_sex_bern_2024) %<-%
  map(
    list(
      "data/abq_demographics_2023.gpkg",
      "data/abq_demographics_2024.gpkg"
    ),
    \(x) st_read(x, layer = "age_sex_bern")
  )

pre_process <- function(df, variable) {
  df %>%
    group_by_at(c("geoid", variable))  %>%
    summarise(value = sum(value)) %>%
    ungroup() %>%
    mutate(area = st_area(.))
}

post_process <- function(df, variable) {
   df %>%
    filter(district != "Los Ranchos") %>% 
    select(-value) %>%
    pivot_wider(names_from = .data[[variable]], values_from = new_value) %>%
    filter(as.numeric(area_pct) > .01) %>%
    select(-c(area, area_split, area_pct))
}

c(abq_age_2023, abq_age_2024) %<-%
  map(list(age_sex_bern_2023, age_sex_bern_2024),
      \(x) x %>% 
        pre_process("agegroup") %>% 
        split_dists() %>% 
        post_process("agegroup"))
```
...
![](images/age_dist_2024.png){width="600"}

![](images/edu_dist_2024.png){width="600"}

I want to look specifically at people under 20 and over 54, so I will calculate the percentage of the population in each district which fall into these groups.
...
A look at the variables shows that the distributions in each district don't look normal, and vary considerably between districts.

```{r}
age_list_str <- list("Under.20.pct", "Over.54.pct")
age_list_dfs <- list(abq_age_2023, abq_age_2024)
age_list_vars <- list(abq_age_2023$Under.20.pct, 
                      abq_age_2023$Over.54.pct,
                      abq_age_2024$Under.20.pct, 
...
c(p1, p2, p3, p4) %<-%
  map2(
    rep(age_list_dfs, each = 2),
    rep(age_list_str, 2),
    \(x, y) x %>%
        ggboxplot("district", y, fill = "district") +
...
In order to analyze the variance, the data should be normally distributed. Lets see if this is so. The null hypothesis in the Shapiro-Wilks test is that the data is normally distributed.

```{r}
map(age_list_vars, shapiro_test)
```

The low p-values suggests that the null hypothesis is false, and the data is not normally distributed, with the exception of the older-than-54 category in 2024.
...
```{r}
#| warning: false
age_list_elf_vars <- list(abq_age_2023$Under.20.pct.ELF, 
                      abq_age_2023$Over.54.pct.ELF,
                      abq_age_2024$Under.20.pct.ELF, 
                      abq_age_2024$Over.54.pct.ELF)

c(p1, p2, p3, p4) %<-%
  map2(
    rep(age_list_dfs, each = 2),
    rep(age_list_elf_str, 2),
    \(x, y) plot_hist(x, y)
  )
c(p5, p6, p7, p8) %<-%
  map(
...
```{r}
map(age_list_elf_vars, shapiro_test)
```
...
```{r}
map2(append(age_list_vars, age_list_elf_vars),
     rep(rep(list(abq_age_2023$district, abq_age_2024$district), each = 2), each = 2),
    \(x, y) bartlett.test(x, y))
```
...
c(aov_under_20_pct_ELF_2023, aov_over_54_pct_ELF_2023,
  aov_under_20_pct_ELF_2024, aov_over_54_pct_ELF_2024) %<-%
  map2(
    rep(age_list_dfs, each = 2),
    rep(age_list_elf_str, 2),
    \(x, y) aov(as.formula(glue("{y} ~ district")),
      data = x
    )
  )

age_aov_list <- list(
  aov_under_20_pct_ELF_2023, aov_over_54_pct_ELF_2023,
  aov_under_20_pct_ELF_2024, aov_over_54_pct_ELF_2024
)

map(age_aov_list, summary)
```

There is evidence of variance in all categories except the children in 2024. There seems to have been a significant change between the two years.

Out of curiosity, I will run the Welch's test on the over 54 group in 2024. This variable was normally distributed, but did not show homogeneity of variance.
...
This supports the result of the `aov` on the transformed data.
