...
widely adopted. While the syntactic standard is excellent, it is not,
unfortunately, a particularly fast library. `collapse`, on the other
hand, while embracing the syntactical conventions of `dplyr` (unlike
`data.table`), focuses on speed. It is orders of magitude faster, and it
is even much faster than `data.table`. Written in C/C++, it has “fast”
versions of many commonly-used `dplyr` functions, simply with a
...
in most cases only one or two function from any given library is needed.
Rather than loading entire packages, I will load the primary libraries
in full, and then the single functions from the other packages. This
approach also has the virtue of declaring all package dependencies at
...
![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-4-1.png)
...
```
    ...
    Unit: microseconds
     expr     min       lq      mean   median      uq      max neval cld
       dp 715.730 742.8410 764.25073 751.8385 763.400 1711.045   100  a 
       co   7.511   9.1615  12.95956  13.0555  16.453   28.740   100   b

The `collapse` version of this very common operation is about 50 times
...
![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-10-1.png)
...
![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-12-1.png)
...
```
    ...
    Unit: microseconds
     expr      min       lq       mean    median        uq      max neval cld
       dp 1725.059 1771.275 1805.86719 1781.9420 1800.6455 3123.293   100  a 
       co   11.999   13.715   19.96373   17.2285   25.4075   59.100   100   b

In this case, `collapse` is nearly 100 times faster.
...
I’ll also make `district` a factor. `qF` is a faster form of `factor`.
...
that `ftransform` allows you to remove columns easily with the syntax
`variable = NULL`.

Finally, I will use `st_cast` to ensure consistent geometries. After
intersection operation, there are likely to be a mix of POLYGONs and
MULTIPOLYGONs, which will cause trouble down the line for the spatial
analyses.
...
```
    ...
    Projected CRS: NAD83(2011) / New Mexico Central
    # A tibble: 16 × 5
       House.Value.Med  Gini Rent.Med Income.Med                            geometry
                 <dbl> <dbl>    <dbl>      <dbl>                  <MULTIPOLYGON [m]>
     1          241200 0.419     956       33875 (((462930.7 453105.8, 462989.7 453…
     2          225100 0.403    1094       87645 (((457618.8 446326, 457657.5 44668…
     3          219100 0.373    1095       48333 (((453727.8 448553, 453809.7 44812…
     4          357700 0.453     942       37088 (((467226.5 458350.1, 467482.7 458…
     5          869000 0.357    1335      250001 (((474183.7 463267.7, 473833.1 463…
     6          271500 0.394    1470.      72926 (((457080.4 467237.4, 457213.1 467…
     7           98900 0.306    1470.      46783 (((454093.8 465851.9, 454092.9 465…
     8          206500 0.592     920.      50329 (((474247.4 451766.8, 474247.4 451…
     9          227650 0.4       867       30810 (((469320.7 450905.4, 469722.4 450…
    10          227650 0.476     861       27168 (((465597.1 454623.1, 465620.1 454…
    11          227650 0.333    1766       83774 (((466956.6 447739.8, 466998.7 447…
    12          206500 0.592    1138       50329 (((474247.5 452850.5, 474251.4 452…
    13          248600 0.476     861       27168 (((465743 454763.3, 465738.4 45475…
    14          248600 0.453     942       37088 (((465992.2 457115, 466122.6 45737…
    15          206500 0.592    1186       50329 (((474243.9 451602.1, 474247.2 451…
    16          251700 0.333    1766       83774 (((474223.8 450039.5, 474223.8 449…
...
```
    ...
    Population           256   2068.5977   2095.4846       0      11152
    ...
    area_pct             256      0.5266      0.4622       0          1
    In.Poverty.Pct       256       5.528      5.0507       0    33.3333
    Population.Density   256   1009.1566   1115.3565       0  5955.4623
    Vacant.Pct           256      4.7517      4.2573       0    20.2716
    Renter.Occupied.Pct  256     34.1214     23.0123       0        100
    Housing.Density      256    476.5967    535.6389       0  2575.2589
...
```
    ...
    Population           173   3058.6763   1862.5095      38      11152
    ...
    area_pct             173      0.7786       0.346  0.0125          1
    In.Poverty.Pct       173         6.5      4.7025       0    22.4014
    Population.Density   173   1492.4446   1058.1767  0.4385  5955.4623
    Vacant.Pct           173      5.5672      4.3509       0    20.2716
    Renter.Occupied.Pct  173     34.3276     21.1954  2.6316    92.3077
    Housing.Density      173    704.8562    513.5391  0.2002  2575.2589
...
![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-23-1.png)


    [[2]]

![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-23-2.png)


    [[3]]

![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-23-3.png)


    [[4]]

![](DistrictVarianceWrangling_files/figure-commonmark/unnamed-chunk-23-4.png)

Finally, I will save the data for use in the next article.
