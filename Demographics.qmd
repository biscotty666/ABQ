...
Finally, I'll summarize the data by district by spatially clipping it to the council districts using `rmapshaper`.
...
cd_demographics <- cd_demographics %>% 
  ...
  mutate(
    ...
    population_pct = population * area_pct,
    in_poverty_pct = in_poverty * area_pct,
    no_hs_deg_pct = no_hs_deg * area_pct,
    hs_some_college_pct = hs_some_college * area_pct,
    bach_assoc_deg_pct = bach_assoc_deg * area_pct,
    grad_deg_pct = grad_deg * area_pct
  ) %>% 
  arrange(GEOID)
```
...
ggplot(council_dists) +
  ...
  geom_sf(data = cd_demographics, aes(fill = in_poverty), alpha = 0.4) +
