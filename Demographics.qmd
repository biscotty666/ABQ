...
cd_demographics <- cd_demographics %>% 
  ...
  mutate(
    ...
    in_poverty = in_poverty / n,
    no_hs_deg = no_hs_deg / n,
    hs_some_college = hs_some_college / n,
    bach_assoc_deg = bach_assoc_deg / n,
    grad_deg = grad_deg / n
  )
```
...
ggplot(council_dists) +
  ...
  geom_sf(data = test, aes(fill = in_poverty), alpha = 0.4) +
...
```{r}
ggplot(cd_demographics, aes(income_hh)) +
  geom_boxplot() +
  coord_flip() +
  facet_wrap(~district) +
  labs(
