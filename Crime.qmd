...
```{r}
incidents_url <- "https://coageo.cabq.gov/cabqgeo/rest/services/Incidents/MapServer/0/query?where=1%3D1&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=true&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&f=kmz"

download.file(incidents_url, "data/incidents.kmz")

unzip("data/incidents.kmz", exdir = "data/")
file.rename("data/doc.kml", "data/incidents.kml")
```

```{r}
incidents <- st_read("data/incidents.kml")
str(incidents)
incidents[1,]
```
